...
//	setlocale(LC_ALL,"");
    ...
    printf("ATENCAO: Limite de 10 caracteres; nao utilize espacos.\n");
    ...
    while(strlen((*user).nome)>10 ||strlen((*user).nome)==0)
	{
		...
		printf("\nDigite um novo nome para o usuario: ");
		scanf("%s",(*user).nome);
	}
}
void solicitarSenha(Cadastro* user)
{
	printf("\n\nATENCAO: Sua senha deve conter exatamente 6 caracteres; nao utilize espacos.\n");
	printf("Digite uma senha: ");
	scanf("%s",(*user).senha);

	while(strlen((*user).senha)<6 || strlen((*user).senha)> 6)
	{
		printf("\nSenha com numero incorreto de caracteres!\n");
